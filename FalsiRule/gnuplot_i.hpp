...
// due to `_popen()` problem with `"` deletion 
std::string to_popen_friendly_str(std::string str) {
	std::replace(str.begin(), str.end(), '\\', '/');
	str = "\"" + str + "\"";
	int g = 0, h = 1, j = 1;
	g = str.find_first_of(" ");
	while (g != -1) {
		h = str.find_last_of("/", g);
		j = str.find_first_of("/", g);
		g = str.find_first_of(" ", g + 1);
		str.insert(h, "\"");
		str.insert(j + 1, "\"");
	};
	return str;
